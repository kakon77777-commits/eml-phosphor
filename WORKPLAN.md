...
## 9. Phase 3 完成（2026-07-18，同一輪接著做）

三項全做完，v0.8.0-beta：

1. **`EAI-RETROFIT.md`**：CTS 六層改寫成領域無關角色（Unit Vocabulary /
   Location Naming / Region Typing / Decoded Content / Intent Annotation /
   Provenance Graph）+ Tier 1/2/3 難度分級，明講 `exhaustive`/`Φ` 保證在不同
   Tier 下是不同強度的承諾。v1 範圍鎖定 Tier 1。
2. **`skills/phosphor-adopt/SKILL.md`**：分類 Tier（不是 Tier 1 就老實說）→
   確認 code/state 是否共用位址空間（WASM vs VM-16 的真實教訓）→ 逐角色提 CTS
   草案、**在人確認前不動手**→ 建 snapshot builder 接現成 `phosphor-stream`
   → 用獨立參照驗證。刻意非一鍵全自動。
3. **`rpn/`**：第二個 Tier 1 目標，堆疊式 RPN 計算機，跟 VM-16/WASM 零共用
   程式碼，證明六角色不是「VM-16 換個名字」。Decoded Content 老實留空，
   Provenance 動態折疊復原。`verify:rpn` → 19 passed，第一次跑就全過。

全套驗證 272 checks，`typecheck` 零錯誤，無回歸。已 commit + push。

**這一輪（Phase 1→2→3）到此告一段落**：WASM 真實目標 → UI（分頁/主題/寵物）→
Phase 2 端到端旗艦案例（真實 rustc + 治理式優化流程）→ Phase 3 通用改造方法論
+ Skill + 第二驗證目標，一路做下來、每步都有真實驗證（跟獨立引擎/獨立計算核對，
不是自我一致）。下一步是什麼，留給 Neo 下次決定。
