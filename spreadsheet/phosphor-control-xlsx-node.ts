/** Node-only reader for the 09_Control worksheet in an .xlsx file. */
...
function texts(xml: string): string {
  return [...xml.matchAll(/<(?:[A-Za-z_][\w.-]*:)?t(?:\s[^>]*)?>([\s\S]*?)<\/(?:[A-Za-z_][\w.-]*:)?t>/g)].map(match => unxml(match[1])).join('');
}
...
function sharedStrings(files: Map<string, Uint8Array>): string[] {
  ...
  return [...source.matchAll(/<(?:[A-Za-z_][\w.-]*:)?si>([\s\S]*?)<\/(?:[A-Za-z_][\w.-]*:)?si>/g)].map(match => texts(match[1]));
}
...
export function controlWorkbookFromXlsxBytes(bytes: Uint8Array): WorkbookModel {
  ...
  const sheet: SheetModel = {
    id: CONTROL_SHEET_ID, name: CONTROL_SHEET_NAME, description: 'Imported XLSX control commands.',
    columns, rows: rows.filter(row => row.some(value => value !== '' && value !== null)),
  };
  return { proto: 'phosphor-sheet-v1', generated_at: '', manifest: {}, sheets: [sheet] };
}

export function readControlCommandsFromXlsxBytes(bytes: Uint8Array) {
  return parseControlSheet(controlWorkbookFromXlsxBytes(bytes));
}

export function readControlCommandsFromXlsx(path: string) {
  return readControlCommandsFromXlsxBytes(new Uint8Array(readFileSync(path)));
}
